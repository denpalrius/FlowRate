    ...
    (function (Controllers) {
        ...
        var AdminController = (function () {
            ...
            AdminController.prototype.addUser = function (isValid) {
                var that = this;
                if (isValid) {
                    that.$scope.adminScope.newUser.id = ThingSpeak.Helpers.AppHelpers.generateGUID();
                    that.FirebaseService.write(ThingSpeak.Configs.AppConfig.firebaseRefs.users, that.$scope.adminScope.newUser)
                        .done(function (response) {
                        ThingSpeak.Helpers.AppHelpers.showToast("User added successfully", true, that.$mdToast);
                    })
                        .fail(function (error) {
                        ThingSpeak.Helpers.AppHelpers.showToast("There was an error adding new user", false, that.$mdToast);
                    });
                }
            };
            ...
            AdminController.prototype.addSensor = function (isValid) {
                ...
                if (isValid) {
                    ...
                    newSensor.id = newSensor.name.replace(/\s/g, '') + '+' + ThingSpeak.Helpers.AppHelpers.generateGUID();
                    that.FirebaseService.write(ThingSpeak.Configs.AppConfig.firebaseRefs.sensors, that.$scope.adminScope.newSensor)
                        .done(function (response) {
                        ThingSpeak.Helpers.AppHelpers.showToast("Sensor added successfully", true, that.$mdToast);
                    })
                        .fail(function (error) {
                        ThingSpeak.Helpers.AppHelpers.showToast("There was an error adding new sensor", false, that.$mdToast);
                    });
                }
            };
            ...
            FirebaseService.prototype.signUp = function (newUser) {
                ...
                firebase.auth().createUserWithEmailAndPassword(newUser.email, newUser.password)
                    ...
                    var loggedInUser = {
                        ...
                        role: ThingSpeak.ViewModels.UserRole.standard
                    };
                    //Add user to Firebase DB
                    var isUserAdded = that.write(ThingSpeak.Configs.AppConfig.firebaseRefs.users, loggedInUser)
                        .done(function (response) {
...
(function (ThingSpeak) {
    ...
    var AppModule = (function () {
        function AppModule() {
            // module
            var ngFlowRate = angular.module("ngFlowRate", ["ngRoute", "ngMaterial", "ngMessages", "ngAnimate", "ui.bootstrap", "ngCookies", "firebase"]);
            // configs
            ...
            //Directives
            ngFlowRate.directive("tsGoogleMap", ["$timeout", "$log", "$rootScope", "HttpService", ThingSpeak.Directives.TsGoogleMap]);
            //Filters
